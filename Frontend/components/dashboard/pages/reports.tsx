...
import { generateMLReport, explainChartOrAlert } from "@/api/services"
...
export function ReportsPage() {
  ...
  const handleGenerateAIReport = async () => {
    ...
    try {
      ...
      setReportNarrative(result.narrative || "No narrative generated.")
  ...
  const handleExplainChart = async () => {
    ...
    try {
      ...
      toast.success("Chart explanation generated")
    } catch (error) {
      console.error("Failed to explain chart", error)
      toast.error("Failed to explain chart")
    } finally {
    ...
    <div className="flex-1 min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <header className="sticky top-0 z-30 backdrop-blur-xl bg-background/80 border-b border-border/50 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Button variant="outline" size="sm" onClick={handleGenerateAIReport} disabled={isGeneratingReport}>
              {isGeneratingReport ? (
            ...
            <Button variant="outline" size="sm" onClick={handleExplainChart} disabled={isExplaining}>
              {isExplaining ? (
                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
              ) : (
                <BarChart3 className="w-4 h-4 mr-2" />
              )}
              {isExplaining ? "Explaining..." : "Explain Chart"}
            </Button>
      ...
      <main className="p-6 space-y-6">
        ...
        {reportNarrative && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Financial summary generated by WealthWise AI</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="prose dark:prose-invert max-w-none whitespace-pre-wrap text-sm">{reportNarrative}</div>
            </CardContent>
          </Card>
        )}
        ...
        {explanation && (
          <Card>
            <CardHeader>
              <CardTitle>Chart Explanation</CardTitle>
              <CardDescription>AI-powered explanation of your financial data</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="prose dark:prose-invert max-w-none whitespace-pre-wrap text-sm">{explanation}</div>
            </CardContent>
          </Card>
        )}
      </main>
