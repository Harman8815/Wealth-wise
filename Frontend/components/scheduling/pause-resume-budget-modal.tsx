...
import { usePauseRecurringBudget, useResumeRecurringBudget } from "@/hooks/use-recurring-budgets"
import { toast } from "@/hooks/use-toast"
import type { RecurringBudget } from "@/api/services"

interface PauseResumeBudgetModalProps {
  open: boolean
 ...
 * budget rule without deleting it.
 */
export function PauseResumeBudgetModal({ open, onOpenChange, rule, resume = false }: PauseResumeBudgetModalProps) {
  const pause = usePauseRecurringBudget()
  const resumeMut = useResumeRecurringBudget()
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {resume ? "Resume" : "Pause"} recurring budget?
          </DialogTitle>
          <DialogDescription>
            {resume
              ? `${rule.name} will resume generating scheduled budgets from its next due date.`
              : `${rule.name} will stop generating budgets until you resume it. No data is lost.`}
          </DialogDescription>
        </DialogHeader>
